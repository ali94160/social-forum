...
const randomComment = async () => {
  return await commentModel.findOne({}).exec();
};
...
describe("User authenticated", () => {
  ...
  describe("User trying to delete non existing comment", () => {
    test("/api/user/comments/" + randomComment._id, async () => {
      const res = await testSession.delete(
        "/api/user/comments/" + randomComment._id + "abc"
      );
  ...
  describe("User creates a comment and then removes it", () => {
    test("/api/user/comments/:id", async () => {
      ...
      const comment = await testSession
        .post("/api/comments")
        .send({ ...newComment, postId });
      const res = await testSession.delete(
        "/api/user/comments/" + comment.body._id
      );
